<?php $__env->startSection('title', isset($announcement) ? 'Edit Pengumuman' : 'Buat Pengumuman'); ?>

<?php $__env->startSection('breadcrumbs'); ?>
    <svg class="w-3 h-3 mx-2 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
    <a href="<?php echo e(route('admin.announcements.index')); ?>" class="hover:text-primary-600 transition-colors">Pengumuman</a>
    <svg class="w-3 h-3 mx-2 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.admin.tajuk-halaman','data' => ['title' => ''.e(isset($announcement) ? 'Edit Pengumuman' : 'Buat Pengumuman Baru').'','backUrl' => ''.e(route('admin.announcements.index')).'']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => ''.e(isset($announcement) ? 'Edit Pengumuman' : 'Buat Pengumuman Baru').'','backUrl' => ''.e(route('admin.announcements.index')).'']); ?>
...
<?php endif; ?>
    ...
    <!-- Form -->
    <form x-data="{
        judul_pengumuman: <?php echo \Illuminate\Support\Js::from((string) old('judul_pengumuman', $announcement->judul_pengumuman ?? ''))->toHtml() ?>,
        targetAudience: <?php echo \Illuminate\Support\Js::from((string) old('target_audience', $announcement->target_audience ?? ''))->toHtml() ?>,
        touched: {},
        contentError: false,
        resetForm() {
            this.judul_pengumuman = <?php echo \Illuminate\Support\Js::from((string) old('judul_pengumuman', $announcement->judul_pengumuman ?? ''))->toHtml() ?>;
            this.targetAudience = <?php echo \Illuminate\Support\Js::from((string) old('target_audience', $announcement->target_audience ?? ''))->toHtml() ?>;
            this.touched = {};
            ...
            setTimeout(() => {
                ...
                if (trixEditor && trixEditor.editor) {
                    trixEditor.editor.loadHTML(<?php echo \Illuminate\Support\Js::from((string) old('isi_pengumuman', $announcement->isi_pengumuman ?? ''))->toHtml() ?>);
                }

                let isPinned = document.querySelector('input[name=diprioritaskan][type=checkbox]');
                if (isPinned) isPinned.checked = <?php echo e(old('diprioritaskan', $announcement->diprioritaskan ?? false) ? 'true' : 'false'); ?>;

                let isActive = document.querySelector('input[name=status_pengumuman][type=checkbox]');
                if (isActive) isActive.checked = <?php echo e(old('status_pengumuman', $announcement->status_pengumuman ?? true) ? 'true' : 'false'); ?>;
            }, 10);
        },
        submitForm(e) {
    ...
    }" @submit="submitForm" novalidate action="<?php echo e(isset($announcement) ? route('admin.announcements.update', $announcement->id) : route('admin.announcements.store')); ?>" method="POST" class="space-y-6">
        <?php echo csrf_field(); ?>
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.admin.input-formulir','data' => ['name' => 'judul_pengumuman','label' => 'Judul Pengumuman','model' => 'judul_pengumuman','placeholder' => 'Contoh: Libur Nasional Idul Fitri 2026','required' => true]] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php $attributes = $attributes->except(\Illuminate\View\AnonymousComponent::ignoredParameterNames()); ?>
<?php endif; ?>
<?php $component->withAttributes(['name' => 'judul_pengumuman','label' => 'Judul Pengumuman','model' => 'judul_pengumuman','placeholder' => 'Contoh: Libur Nasional Idul Fitri 2026','required' => true]); ?>
<?php echo $__env->renderComponent(); ?>
<?php endif; ?>
...
<?php endif; ?>
                        ...
                        <div :class="contentError ? 'has-error' : ''" @trix-change="if (contentError) contentError = document.getElementById('isi_pengumuman').value.replace(/<[^>]*>?/gm, '').trim() === ''">
                            ...
                            <input id="isi_pengumuman" type="hidden" name="isi_pengumuman" value="<?php echo e(old('isi_pengumuman', $announcement->isi_pengumuman ?? '')); ?>">
                            <div x-ignore>
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.antarmuka.galat-sebaris','data' => ['name' => 'isi_pengumuman']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php $attributes = $attributes->except(\Illuminate\View\AnonymousComponent::ignoredParameterNames()); ?>
<?php endif; ?>
<?php $component->withAttributes(['name' => 'isi_pengumuman']); ?>
<?php echo $__env->renderComponent(); ?>
<?php endif; ?>
            ...
            <div class="space-y-5">
                    ...
                    <div class="space-y-4">
                        <label class="flex items-start p-3 border border-gray-100 rounded-xl hover:bg-gray-50 transition-colors cursor-pointer group">
                            <div class="flex items-center h-5 mt-0.5">
                                ...
                                <input type="checkbox" name="status_pengumuman" value="1" <?php echo e(old('status_pengumuman', $announcement->status_pengumuman ?? true) ? 'checked' : ''); ?> class="w-4 h-4 text-primary-600 bg-white border-gray-300 rounded focus:ring-primary-500 focus:ring-2 cursor-pointer">
                            </div>
                        ...
                        <label class="flex items-start p-3 border border-gray-100 rounded-xl hover:bg-gray-50 transition-colors cursor-pointer group">
                            <div class="flex items-center h-5 mt-0.5">
                                ...
                                <input type="checkbox" name="diprioritaskan" value="1" <?php echo e(old('diprioritaskan', $announcement->diprioritaskan ?? false) ? 'checked' : ''); ?> class="w-4 h-4 text-amber-500 bg-white border-gray-300 rounded focus:ring-amber-500 focus:ring-2 cursor-pointer">
                            </div>
                            <div class="ml-3 text-sm">
                ...
                <div class="flex flex-col gap-4 mt-8">
                    <button type="submit" class="w-full flex items-center justify-center px-8 py-4 text-base font-extrabold text-white transition-all duration-100 bg-primary-600 rounded-2xl hover:bg-primary-700 shadow-lg shadow-primary-500/30 transform hover:-translate-y-1">
                        ...
                        <?php echo e(isset($announcement) ? 'Simpan Perubahan' : 'Terbitkan Pengumuman'); ?>
                    ...
                    <button type="button" @click="resetForm()" class="w-full flex items-center justify-center px-6 py-4 text-sm font-bold text-gray-700 transition-all duration-100 bg-white border border-gray-300 rounded-2xl hover:bg-red-50 hover:border-red-200 hover:text-red-600 shadow-sm transform hover:-translate-y-1">
                        ...
                        <?php echo e(isset($announcement) ? 'Reset Perubahan' : 'Bersihkan Data'); ?>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\ruang-les\resources\views/admin/pengumuman/formulir.blade.php ENDPATH**/ ?>