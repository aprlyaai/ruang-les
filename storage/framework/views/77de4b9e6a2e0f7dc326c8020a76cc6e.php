...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('toastNotification', () => ({
            toasts: [],
            lastPollTime: <?php echo e(now()->timestamp); ?>, // Waktu inisialisasi dari server
            init() {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        <?php if(session('success')): ?>
            setTimeout(() => {
                window.dispatchEvent(new CustomEvent('notify', {
                    detail: { type: 'success', title: '✨ Yeaayy, Sukses!', text: <?php echo \Illuminate\Support\Js::from(session('success'))->toHtml() ?>, duration: 4500 }
                }));
            }, 300);
        <?php endif; ?>
        ...
        <?php if(session('error')): ?>
            setTimeout(() => {
                window.dispatchEvent(new CustomEvent('notify', {
                    detail: { type: 'error', title: '🚨 Yahh, Terjadi Kesalahan!', text: <?php echo \Illuminate\Support\Js::from(session('error'))->toHtml() ?>, duration: 5000 }
                }));
            }, 300);
        <?php endif; ?>
    });
</script>
<?php /**PATH C:\laragon\www\ruang-les\resources\views/components/notifikasi-singkat.blade.php ENDPATH**/ ?>