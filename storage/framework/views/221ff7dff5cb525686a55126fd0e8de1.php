<?php if (isset($component)) { $__componentOriginalba045c1307213fb91ae4e52604fa7c6c = $component; } ?>
<?php if (isset($attributes)) { $__attributesOriginalba045c1307213fb91ae4e52604fa7c6c = $attributes; } ?>
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.tata-letak-aplikasi','data' => []] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
<?php $component->withName('tata-letak-aplikasi'); ?>
...
?>
    ...
    <div class="relative w-full min-h-screen overflow-x-hidden flex flex-col justify-start pt-38 pb-22 px-4 sm:px-6 lg:px-8">
        ...
        <div class="px-2 md:px-8 max-w-4xl mx-auto">
            <div class="relative w-full">
                ...
                <div class="absolute left-[calc(100%/14)] top-7 md:top-8 transform -translate-y-1/2 h-[3px] bg-gradient-to-r from-primary-400 to-primary-600 z-0 transition-all duration-500" style="width: calc((100% / 7) * <?php echo e($draft->current_step - 1); ?>);"></div>
                ...
                <div class="grid grid-cols-7 w-full relative z-10">
                    <?php for($i = 1; $i <= 7; $i++): ?>
                        ...
                        <div class="h-14 md:h-16 flex items-center justify-center relative z-10 mb-2 md:mb-3">
                            <?php if($i < $draft->current_step): ?>
                                <!-- Completed Step (Clickable) -->
                                <button type="submit" name="action" value="jump_<?php echo e($i); ?>" form="formPendaftaran" formnovalidate class="w-10 h-10 md:w-12 md:h-12 rounded-full bg-gradient-to-tr from-primary-600 to-primary-400 text-white flex items-center justify-center font-bold shadow-md shadow-primary-500/30 ring-4 ring-primary-100 transform transition-all duration-300 hover:scale-110 hover:ring-primary-200 cursor-pointer focus:outline-none focus:ring-primary-300">
                            ...
                            <?php elseif($i == $draft->current_step): ?>
                                <!-- Current Step -->
                        ...
                        <div class="hidden sm:flex w-full items-start justify-center px-1">
                            <?php if($i < $draft->current_step): ?>
                                <span class="text-[10px] md:text-xs font-semibold text-primary-600 text-center leading-tight transition-colors"><?php echo e($stepNames[$i]); ?></span>
                            <?php elseif($i == $draft->current_step): ?>
                                <span class="text-[10px] md:text-xs font-extrabold text-primary-800 text-center leading-tight transition-colors"><?php echo e($stepNames[$i]); ?></span>
                            <?php else: ?>
                                <span class="text-[10px] md:text-xs font-medium text-gray-400 text-center leading-tight transition-colors"><?php echo e($stepNames[$i]); ?></span>
                            <?php endif; ?>
                        </div>
                    </div>
                <?php endfor; ?>
        ...
        <div class="mt-12 bg-white/80 backdrop-blur-xl rounded-3xl shadow-[0_8px_30px_rgb(66,108,60,0.15)] overflow-hidden border border-white/50 transition-all duration-500 hover:shadow-[0_20px_50px_rgb(66,108,60,0.2)]">
            ...
            <div class="bg-white/50 backdrop-blur-md px-6 py-5 border-b border-gray-200/50 flex items-center gap-4">
                <div class="w-12 h-12 sm:w-12 sm:h-12 bg-gradient-to-br from-primary-500 to-primary-700 rounded-xl sm:rounded-2xl text-white flex items-center justify-center font-extrabold text-xl sm:text-2xl shrink-0 shadow-lg shadow-primary-500/30">
                    <?php echo e($draft->current_step); ?>

                </div>
                <div>
                    <h2 class="text-lg md:text-xl font-extrabold text-slate-900 font-heading"><?php echo e($stepNames[$draft->current_step]); ?></h2>
                    <p class="text-xs md:text-sm text-gray-500 mt-0.5"><?php echo e($stepInstructions[$draft->current_step]); ?></p>
                </div>
            </div>

            <!-- Card Body Form -->
            <div class="p-6 md:p-8">
                <form id="formPendaftaran" action="<?php echo e(route('pendaftaran.save')); ?>" method="POST" enctype="multipart/form-data" novalidate>
                    <?php echo csrf_field(); ?>



                    <?php if($draft->current_step == 1): ?>
                        <?php echo $__env->make('pendaftaran.langkah1-identitas', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
                    <?php elseif($draft->current_step == 2): ?>
                        <?php echo $__env->make('pendaftaran.langkah2-akademik', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
                    <?php elseif($draft->current_step == 3): ?>
                        <?php echo $__env->make('pendaftaran.langkah3-ortu', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
                    <?php elseif($draft->current_step == 4): ?>
                        <?php echo $__env->make('pendaftaran.langkah4-paket', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
                    <?php elseif($draft->current_step == 5): ?>
                        <?php echo $__env->make('pendaftaran.langkah5-jadwal', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
                    <?php elseif($draft->current_step == 6): ?>
                        <?php echo $__env->make('pendaftaran.langkah6-review', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
                    <?php elseif($draft->current_step == 7): ?>
                        <?php echo $__env->make('pendaftaran.langkah7-pembayaran', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
                    <?php endif; ?>
                    ...
                    <div class="mt-10 flex flex-col-reverse sm:flex-row items-center justify-between pt-6 border-t-2 border-gray-200 gap-4">
                        ...
                        <div class="w-full sm:w-1/3 flex justify-center">
                            <span class="text-sm font-bold text-gray-500 bg-white/50 px-4 py-1 rounded-full border border-white/60 shadow-inner">Langkah <?php echo e($draft->current_step); ?> dari 7</span>
                        </div>

                        <!-- Sisi Kanan: Tombol Lanjut/Selesai -->
                        <div class="w-full sm:w-1/3 flex justify-end">
                            <?php if($draft->current_step == 7): ?>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Menghilangkan pesan error dan styling merah saat user mulai mengetik/mengubah nilai
            const formPendaftaran = document.getElementById('formPendaftaran');
            if (formPendaftaran) {
                const errorFields = formPendaftaran.querySelectorAll('.border-red-300, .bg-red-50');
                
                errorFields.forEach(field => {
                    const removeErrorState = function() {
                        // Hapus class error pada elemen input/select
                        this.classList.remove('border-red-300', 'ring-1', 'ring-red-300', 'bg-red-50');
                        
                        // Cari dan hilangkan pesan error di bawahnya (komponen galat-sebaris)
                        const container = this.closest('div');
                        if (container) {
                            const errorMsg = container.querySelector('p.text-red-500');
                            if (errorMsg) {
                    ...
                    field.addEventListener('input', removeErrorState, { once: true });
                    field.addEventListener('change', removeErrorState, { once: true });
                });
            }
                ...
                inputs.forEach(input => {
                    input.addEventListener('change', function() {
                        ...
                        autosaveTimeout = setTimeout(() => {
                            ...
                            fetch('<?php echo e(route("pendaftaran.autosave")); ?>', {
                                method: 'POST',
                                headers: {
...
<?php endif; ?>
<?php if (isset($__attributesOriginalba045c1307213fb91ae4e52604fa7c6c)): ?>
<?php $attributes = $__attributesOriginalba045c1307213fb91ae4e52604fa7c6c; ?>
<?php unset($__attributesOriginalba045c1307213fb91ae4e52604fa7c6c); ?>
<?php endif; ?>
<?php if (isset($__componentOriginalba045c1307213fb91ae4e52604fa7c6c)): ?>
<?php $component = $__componentOriginalba045c1307213fb91ae4e52604fa7c6c; ?>
<?php unset($__componentOriginalba045c1307213fb91ae4e52604fa7c6c); ?>
<?php endif; ?>

<?php /**PATH C:\laragon\www\ruang-les\resources\views/publik/pendaftaran.blade.php ENDPATH**/ ?>