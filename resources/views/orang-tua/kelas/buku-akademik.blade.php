...
<div class="space-y-6 w-full" x-data="{ activeTab: 'presensi' }">
    ...
    <div class="mb-6">
        <x-admin.tajuk-halaman
            title="Buku Akademik"
            description="Pantau riwayat presensi, catatan perkembangan, dan nilai anak secara menyeluruh."
        />
    </div>
                ...
                @else
                    <div class="overflow-x-auto">
                        <table class="w-full text-left border-collapse">
                            ...
                            <tbody class="divide-y divide-gray-100">
                                @foreach($scores as $skor_nilai)
                                    <tr class="hover:bg-gray-50/50 transition-colors">
                                        <td class="px-6 py-4 align-top">
                                            <p class="text-sm font-semibold text-gray-900">{{ \Carbon\Carbon::parse($skor_nilai->tanggal_penilaian)->translatedFormat('l, d F Y') }}</p>
                                            <p class="text-sm font-semibold text-primary-700 mt-1">{{ $skor_nilai->tipe_nilai }}</p>
                                        </td>
                                        <td class="px-6 py-4 align-top">
                                            <p class="text-sm font-semibold text-gray-900">{{ $skor_nilai->materi_nilai }}</p>
                                        </td>
                                        <td class="px-4 py-3 align-middle text-center">
                                            <div class="inline-flex items-center justify-center w-10 h-10 rounded-full {{ $skor_nilai->skor_nilai >= 80 ? 'bg-primary-50 text-primary-600' : ($skor_nilai->skor_nilai >= 60 ? 'bg-yellow-50 text-yellow-600' : 'bg-red-50 text-red-600') }}">
                                                <span class="text-base font-black leading-none">{{ $skor_nilai->skor_nilai }}</span>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4 align-top">
                                            @if($skor_nilai->notes_nilai)
                                                <p class="text-sm text-gray-600 italic">"{{ $skor_nilai->notes_nilai }}"</p>
                                            @else
                                                <span class="text-sm text-gray-600">-</span>
                                            @endif
                                        </td>
