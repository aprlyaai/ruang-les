...
@section('breadcrumbs')
    ...
    <a href="{{ route('ortu.layanan.index') }}" class="text-gray-500 hover:text-primary-600 transition-colors font-medium">Layanan & Bantuan</a>
    <svg class="w-3 h-3 mx-2 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
...
<div class="flex flex-col min-h-[calc(100vh-11rem)]">
    ...
    <div class="shrink-0 bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col sm:flex-row justify-between items-start gap-4 mb-6">
        ...
        <div class="flex items-center flex-wrap gap-3">
            ...
            <form action="{{ route('ortu.layanan.close', $ticket->id) }}" method="POST" class="close-ticket-form">
                @csrf
    ...
    <div class="shrink-0 bg-white rounded-2xl shadow-sm border border-gray-100 p-4 sm:p-6">
        <form action="{{ route('ortu.layanan.reply', $ticket->id) }}" method="POST" x-data="{ contentError: false }" @submit="if(document.getElementById('pesan').value.replace(/<[^>]*>?/gm, '').trim() === '') { contentError = true; $event.preventDefault(); }">
            @csrf
            <div class="mb-5">
