<header class="h-20 bg-white/80 backdrop-blur-md border border-primary-100/50 rounded-2xl flex items-center justify-between px-6 shadow-sm relative z-20">
    <div class="flex items-center">
        ...
        <div class="hidden sm:flex items-center text-sm font-medium text-gray-500">
            ...
            <a href="{{ route('mentor.dashboard') }}" class="hover:text-primary-600 transition-colors">Home</a>
    ...
    <div class="flex items-center space-x-4">
        ...
        <div x-data="{ dropdownOpen: false }" class="relative">
            <button @click="dropdownOpen = !dropdownOpen" class="flex items-center space-x-3 focus:outline-none px-3 py-2 rounded-xl hover:bg-white transition-colors border border-transparent hover:border-gray-100 hover:shadow-sm">
                ...
                @if(Auth::user()->avatar)
                    <img src="{{ asset('storage/' . Auth::user()->avatar) }}" alt="{{ Auth::user()->name }}" class="w-9 h-9 rounded-lg object-cover shadow-sm border border-primary-100 flex-shrink-0">
                @else
            ...
            <div x-show="dropdownOpen" 
                ...
                <a href="{{ route('mentor.profile.index') }}" class="flex items-center px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 hover:text-primary-600 font-medium transition-colors group">
