...
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-4 sm:space-y-0">
    <div class="flex items-center space-x-4">
        @if($backUrl)
        <a href="{{ $backUrl }}" class="w-9 h-9 flex items-center justify-center text-gray-500 hover:text-primary-700 hover:bg-primary-50 bg-white rounded-full shadow-sm border border-gray-200 hover:border-primary-200 transition-all duration-100 transform hover:-translate-y-0.5 shrink-0" title="Kembali">
    ...
    @if($actionUrl)
    <div>
        <a href="{{ $actionUrl }}" class="inline-flex items-center justify-center px-4 py-2 text-sm font-bold text-white transition-all duration-100 bg-primary-600 rounded-xl hover:bg-primary-700 shadow-sm hover:-translate-y-0.5 whitespace-nowrap">
            @if($icon == 'add')
