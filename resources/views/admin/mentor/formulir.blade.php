...
@section('title', isset($mentor) ? 'Edit Mentor' : 'Tambah Mentor')
...
@section('breadcrumbs')
    ...
    @if(isset($mentor) && request('from') == 'detail')
        <svg class="w-3 h-3 mx-2 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
        <a href="{{ route('admin.mentor.show', $mentor->id) }}" class="hover:text-primary-600 transition-colors">Profil Mentor</a>
    @endif
    <svg class="w-3 h-3 mx-2 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
...
<div class="w-full space-y-6">
    ...
    <x-admin.tajuk-halaman
        title="{{ isset($mentor) ? 'Edit Data Mentor' : 'Tambah Mentor Baru' }}"
        backUrl="{{ (isset($mentor) && request('from') == 'detail') ? route('admin.mentor.show', $mentor->id) : route('admin.mentor.index') }}"
    />

        <!-- Form -->
    <form action="{{ isset($mentor) ? route('admin.mentor.update', $mentor->id) : route('admin.mentor.store') }}" method="POST" enctype="multipart/form-data"
          x-data="{
              from: '{{ request('from') }}',
              name: @js((string) old('name', $mentor->name ?? '')),
              email: @js((string) old('email', $mentor->email ?? '')),
              phoneNumber: @js((string) old('no_telepon_mentor', $mentor->mentorProfile->no_telepon_mentor ?? '')),
              showPassword: false,
              showPasswordConfirm: false,
              password: '',
              birthPlace: @js((string) old('tempat_lahir_mentor', $mentor->mentorProfile->tempat_lahir_mentor ?? '')),
              birthDate: @js((string) old('tanggal_lahir_mentor', $mentor->mentorProfile->tanggal_lahir_mentor ?? '')),
              jenis_kelamin_mentor: @js((string) old('jenis_kelamin_mentor', $mentor->mentorProfile->jenis_kelamin_mentor ?? '')),
              alamat_mentor: @js((string) old('alamat_mentor', $mentor->mentorProfile->alamat_mentor ?? '')),
              education: @js((string) old('pendidikan_mentor', $mentor->mentorProfile->pendidikan_mentor ?? '')),
              isActive: @js((string) old('status_mentor', $mentor->mentorProfile->status_mentor ?? '')),
              imagePreview: @js(isset($mentor) && $mentor->avatar ? asset('storage/' . $mentor->avatar) : ''),
              touched: {},
              ...
              resetForm() {
                  this.name = @js((string) old('name', $mentor->name ?? ''));
                  this.email = @js((string) old('email', $mentor->email ?? ''));
                  this.phoneNumber = @js((string) old('no_telepon_mentor', $mentor->mentorProfile->no_telepon_mentor ?? ''));
                  this.password = '';
                  this.showPassword = false;
                  this.showPasswordConfirm = false;
                  this.birthPlace = @js((string) old('tempat_lahir_mentor', $mentor->mentorProfile->tempat_lahir_mentor ?? ''));
                  this.birthDate = @js((string) old('tanggal_lahir_mentor', $mentor->mentorProfile->tanggal_lahir_mentor ?? ''));
                  this.jenis_kelamin_mentor = @js((string) old('jenis_kelamin_mentor', $mentor->mentorProfile->jenis_kelamin_mentor ?? ''));
                  this.alamat_mentor = @js((string) old('alamat_mentor', $mentor->mentorProfile->alamat_mentor ?? ''));
                  this.education = @js((string) old('pendidikan_mentor', $mentor->mentorProfile->pendidikan_mentor ?? ''));
                  this.isActive = @js((string) old('status_mentor', $mentor->mentorProfile->status_mentor ?? ''));
                  this.imagePreview = @js(isset($mentor) && $mentor->avatar ? asset('storage/' . $mentor->avatar) : '');
                  this.touched = {};

                  setTimeout(() => {
                      let ts = document.querySelector('input[name=spesialisasi_mentor]');
                      if (ts) ts.value = @js((string) old('spesialisasi_mentor', $mentor->mentorProfile->spesialisasi_mentor ?? ''));

                      let bn = document.querySelector('input[name=nama_bank]');
                      if (bn) bn.value = @js((string) old('nama_bank', $mentor->mentorProfile->nama_bank ?? ''));

                      let ban = document.querySelector('input[name=nomor_akun_bank]');
                      if (ban) ban.value = @js((string) old('nomor_akun_bank', $mentor->mentorProfile->nomor_akun_bank ?? ''));

                      let bname = document.querySelector('input[name=nama_akun_bank]');
                      if (bname) bname.value = @js((string) old('nama_akun_bank', $mentor->mentorProfile->nama_akun_bank ?? ''));
        ...
        <input type="hidden" name="from" :value="from">
                    ...
                    <div class="w-full lg:w-2/3 xl:w-3/4">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                            ...
                            <div>
                                ...
                                <x-antarmuka.galat-sebaris name="no_telepon_mentor" />
                            </div>
                            <div>
                                <label class="block text-sm text-gray-600 font-semibold mb-2">Password {!! isset($mentor) ? '<span class="text-xs font-normal text-gray-500 ml-1">(Kosongkan jika tidak diubah)</span>' : '<span class="text-red-500">*</span>' !!}</label>
                                <div class="relative">
                                    <input :type="showPassword ? 'text' : 'password'" name="password" x-model="password" placeholder="Masukkan password minimal 8 karakter" @blur="touched.password = true" :class="touched.password && password === '' && !{{ isset($mentor) ? 'true' : 'false' }} ? 'border-red-500 focus:ring-red-500 bg-red-50/30' : 'border-gray-200 focus:border-primary-400 bg-gray-50'" class="block w-full rounded-2xl p-3 pr-10 border shadow-sm focus:bg-white focus:outline-none focus:ring-2 focus:ring-primary-200 transition-colors duration-100 text-sm font-medium text-gray-800" {{ isset($mentor) ? '' : 'required' }}>
                            ...
                            <div>
                                <label class="block text-sm text-gray-600 font-semibold mb-2">Konfirmasi Password {!! isset($mentor) ? '' : '' !!}</label>
                                <div class="relative">
                                    <input :type="showPasswordConfirm ? 'text' : 'password'" name="password_confirmation" placeholder="Ulangi password yang telah dibuat" class="block w-full rounded-2xl p-3 pr-10 border border-gray-200 bg-gray-50 shadow-sm focus:bg-white focus:outline-none focus:ring-2 focus:ring-primary-200 focus:border-primary-400 transition-colors duration-100 text-sm font-medium text-gray-800" {{ isset($mentor) ? '' : 'required' }}>
                    ...
                    <div class="space-y-5">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <div>
                                ...
                                <x-antarmuka.galat-sebaris name="tempat_lahir_mentor" />
                            </div>
                            <div>
                                <label class="block text-sm text-gray-600 font-semibold mb-2">Tanggal Lahir <span class="text-red-500">*</span></label>
                                <input type="date" name="tanggal_lahir_mentor" max="{{ date('Y-m-d') }}" x-model="birthDate" @blur="touched.birthDate = true" :class="touched.birthDate && birthDate.trim() === '' ? 'border-red-500 focus:ring-red-500 bg-red-50/30' : 'border-gray-200 focus:border-primary-400 bg-gray-50'" class="block w-full rounded-2xl p-3 border shadow-sm focus:bg-white focus:outline-none focus:ring-2 focus:ring-primary-200 transition-colors duration-100 text-sm font-medium text-gray-800" required>
                                ...
                                <x-antarmuka.galat-sebaris name="tanggal_lahir_mentor" />
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="jenis_kelamin_mentor" x-model="jenis_kelamin_mentor" @blur="touched.jenis_kelamin_mentor = true" :class="touched.jenis_kelamin_mentor && jenis_kelamin_mentor.trim() === '' ? 'border-red-500 focus:ring-red-500 bg-red-50/30' : 'border-gray-200 focus:border-primary-400 bg-gray-50'" class="block w-full appearance-none rounded-2xl p-3 pr-10 border shadow-sm focus:bg-white focus:outline-none focus:ring-2 focus:ring-primary-200 transition-colors duration-100 text-sm font-medium text-gray-800 bg-[url('data:image/svg+xml;charset=US-ASCII,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%2224%22%20height%3D%2224%22%20viewBox%3D%220%200%2024%2024%22%20fill%3D%22none%22%20stroke%3D%22%234b5563%22%20stroke-width%3D%222%22%20stroke-linecap%3D%22round%22%20stroke-linejoin%3D%22round%22%3E%3Cpolyline%20points%3D%226%209%2012%2015%2018%209%22%3E%3C%2Fpolyline%3E%3C%2Fsvg%3E')] bg-[length:1.25rem_1.25rem] bg-[position:right_1rem_center] bg-no-repeat cursor-pointer" required>
                                <option value="" disabled {{ !isset($mentor) && !old('jenis_kelamin_mentor') ? 'selected' : '' }}>Pilih Jenis Kelamin</option>
                                <option value="Laki-laki" {{ old('jenis_kelamin_mentor', $mentor->mentorProfile->jenis_kelamin_mentor ?? '') == 'Laki-laki' ? 'selected' : '' }}>Laki-laki</option>
                                <option value="Perempuan" {{ old('jenis_kelamin_mentor', $mentor->mentorProfile->jenis_kelamin_mentor ?? '') == 'Perempuan' ? 'selected' : '' }}>Perempuan</option>
                            </select>
                            ...
                            <x-antarmuka.galat-sebaris name="jenis_kelamin_mentor" />
                        </div>
                        ...
                        <div>
                            ...
                            <x-antarmuka.galat-sebaris name="alamat_mentor" />
                        </div>
                    </div>
                </div>
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-primary-100/50 p-5 md:p-6">
                        ...
                        <div class="space-y-5">
                            <div>
                                ...
                                <x-antarmuka.galat-sebaris name="pendidikan_mentor" />
                            </div>
                            <div>
                                <label class="block text-sm text-gray-600 font-semibold mb-2">Spesialisasi Mengajar</label>
                                <input type="text" name="spesialisasi_mentor" value="{{ old('spesialisasi_mentor', $mentor->mentorProfile->spesialisasi_mentor ?? '') }}" placeholder="Contoh: Mata pelajaran SD" class="block w-full rounded-2xl p-3 border border-gray-200 bg-gray-50 shadow-sm focus:bg-white focus:outline-none focus:ring-2 focus:ring-primary-200 focus:border-primary-400 transition-colors duration-100 text-sm font-medium text-gray-800">
                            </div>
                            <div>
                                ...
                                <select name="status_mentor" x-model="isActive" @blur="touched.isActive = true" :class="touched.isActive && isActive === '' ? 'border-red-500 focus:ring-red-500 bg-red-50/30' : 'border-gray-200 focus:border-primary-400 bg-gray-50'" class="block w-full appearance-none rounded-2xl p-3 pr-10 border shadow-sm focus:bg-white focus:outline-none focus:ring-2 focus:ring-primary-200 transition-colors duration-100 text-sm font-medium text-gray-800 bg-[url('data:image/svg+xml;charset=US-ASCII,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%2224%22%20height%3D%2224%22%20viewBox%3D%220%200%2024%2024%22%20fill%3D%22none%22%20stroke%3D%22%234b5563%22%20stroke-width%3D%222%22%20stroke-linecap%3D%22round%22%20stroke-linejoin%3D%22round%22%3E%3Cpolyline%20points%3D%226%209%2012%2015%2018%209%22%3E%3C%2Fpolyline%3E%3C%2Fsvg%3E')] bg-[length:1.25rem_1.25rem] bg-[position:right_1rem_center] bg-no-repeat cursor-pointer" required>
                                    <option value="" disabled {{ !isset($mentor) && !old('status_mentor') ? 'selected' : '' }}>Pilih Status Keaktifan</option>
                                    <option value="1" {{ old('status_mentor', $mentor->mentorProfile->status_mentor ?? '') == '1' ? 'selected' : '' }}>Aktif Mengajar</option>
                                    <option value="0" {{ old('status_mentor', $mentor->mentorProfile->status_mentor ?? '') == '0' ? 'selected' : '' }}>Nonaktif</option>
                                </select>
                                ...
                                <x-antarmuka.galat-sebaris name="status_mentor" />
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-primary-100/50 p-5 md:p-6 w-full">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="nama_akun_bank" value="{{ old('nama_akun_bank', $mentor->mentorProfile->nama_akun_bank ?? '') }}" placeholder="Contoh: Ruang Les by Ismaturrohmah" class="block w-full rounded-xl p-3 border border-gray-200 bg-gray-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-primary-200 focus:border-primary-400 transition-colors duration-100 text-sm font-medium text-gray-800">
            ...
            <div class="flex flex-col-reverse md:flex-row gap-4 mt-8">
                ...
                <button type="submit" class="w-full md:w-2/3 flex items-center justify-center px-8 py-4 text-base font-extrabold text-white transition-all duration-100 bg-primary-600 rounded-2xl hover:bg-primary-700 shadow-lg shadow-primary-500/30 transform hover:-translate-y-1">
                    ...
                    {{ isset($mentor) ? 'Simpan Perubahan Data Mentor' : 'Simpan Data Mentor Baru' }}
                </button>
            </div>
        </div>
    </form>
