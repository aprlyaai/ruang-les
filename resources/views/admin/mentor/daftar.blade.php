...
<div class="space-y-6">
    ...
    <x-admin.tajuk-halaman
        title="Daftar Mentor Ruang Les"
        description="Pusat informasi untuk mengenal dan mengelola jajaran pengajar Ruang Les."
        actionUrl="{{ route('admin.mentor.create') }}"
            ...
            <table class="w-full text-left border-collapse text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($mentors as $mentor)
                        <tr class="hover:bg-gray-50 transition-colors" x-show="(filter === 'all' || filter === '{{ optional($mentor->mentorProfile)->status_mentor ? '1' : '0' }}') && (search === '' || @js(strtolower($mentor->name ?? '')).includes(search.toLowerCase()) || @js(strtolower($mentor->email ?? '')).includes(search.toLowerCase()))" x-transition>
                            <td class="px-4 py-3 align-middle">
                                <div class="flex items-center">
                                    <div class="flex-shrink-0">
                                        <x-admin.avatar :name="$mentor->name" :avatar-url="$mentor->avatar" />
                                    </div>
                                    <div class="ml-4">
                                        <div class="text-sm font-bold text-gray-900">{{ $mentor->name }}</div>
                                        <div class="text-xs text-gray-500 mt-1">{{ $mentor->mentorProfile->spesialisasi_mentor ?? 'Belum ada spesialisasi' }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-4 py-3 align-middle">
                                ...
                                <div class="text-xs text-gray-500 flex items-center mt-1">
                                    <svg class="w-4 h-4 text-primary-600 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"></path></svg>
                                    {{ $mentor->mentorProfile->no_telepon_mentor ?? '-' }}
                                </div>
                            </td>
                            <td class="px-4 py-3 align-middle">
                            ...
                            <td class="px-4 py-3 align-middle">
                                <div class="flex items-center justify-center space-x-2">
                                    <a href="{{ route('admin.mentor.show', $mentor->id) }}" class="inline-flex items-center p-2 text-sm font-medium text-gray-500 bg-gray-50 rounded-lg hover:bg-gray-100 hover:text-primary-600 transition-colors focus:outline-none focus:ring-2 focus:ring-primary-500" title="Lihat Profil Mentor">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                    </a>
                                    <a href="{{ route('admin.mentor.edit', $mentor->id) }}" class="inline-flex items-center p-2 text-sm font-medium text-gray-500 bg-gray-50 rounded-lg hover:bg-gray-100 hover:text-primary-600 transition-colors focus:outline-none focus:ring-2 focus:ring-primary-500" title="Edit">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                    </a>
                                    <x-admin.formulir-hapus :route="route('admin.mentor.destroy', $mentor->id)" :item-name="$mentor->name" />
                                </div>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-12 align-middle">
                                <x-admin.keadaan-kosong
                                    ...
                                    message="Saat ini belum ada mentor yang bergabung maupun ditambahkan."
                                />
                            </td>
                        </tr>
                    @endforelse
