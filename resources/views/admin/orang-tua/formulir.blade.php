...
@section('title', isset($parent) ? 'Edit Wali Murid' : 'Tambah Wali Murid')

@section('breadcrumbs')
    <svg class="w-3 h-3 mx-2 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
    <a href="{{ route('admin.parents.index') }}" class="hover:text-primary-600 transition-colors">Data Wali Murid</a>
    @if(isset($parent) && request('from') == 'detail')
        <svg class="w-3 h-3 mx-2 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
        <a href="{{ route('admin.parents.show', $parent->id) }}" class="hover:text-primary-600 transition-colors">Profil Wali Murid</a>
    @endif
    <svg class="w-3 h-3 mx-2 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
...
<div class="w-full space-y-6">
    ...
    <x-admin.tajuk-halaman
        title="{{ isset($parent) ? 'Edit Data Wali Murid' : 'Tambah Wali Murid Baru' }}"
        backUrl="{{ (isset($parent) && request('from') == 'detail') ? route('admin.parents.show', $parent->id) : route('admin.parents.index') }}"
    />

    <!-- Form -->
    <form action="{{ isset($parent) ? route('admin.parents.update', $parent->id) : route('admin.parents.store') }}" method="POST" enctype="multipart/form-data" novalidate
          x-data="{
              name: @js((string) old('name', $parent->name ?? '')),
              email: @js((string) old('email', $parent->email ?? '')),
              phoneNumber: @js((string) old('no_telepon_orangtua', $parent->parentProfile->no_telepon_orangtua ?? '')),
              showPassword: false,
              showPasswordConfirm: false,
              password: '',
              statusHubungan: @js((string) old('status_hubungan', $parent->parentProfile->status_hubungan ?? '')),
              alamatDomisili: @js((string) old('alamat_domisili', $parent->parentProfile->alamat_domisili ?? '')),
              imagePreview: @js(isset($parent) && $parent->avatar ? asset('storage/' . $parent->avatar) : ''),
              touched: {},
              ...
              resetForm() {
                  this.name = @js((string) old('name', $parent->name ?? ''));
                  this.email = @js((string) old('email', $parent->email ?? ''));
                  this.phoneNumber = @js((string) old('no_telepon_orangtua', $parent->parentProfile->no_telepon_orangtua ?? ''));
                  this.password = '';
                  this.showPassword = false;
                  this.showPasswordConfirm = false;
                  this.statusHubungan = @js((string) old('status_hubungan', $parent->parentProfile->status_hubungan ?? ''));
                  this.alamatDomisili = @js((string) old('alamat_domisili', $parent->parentProfile->alamat_domisili ?? ''));
                  this.imagePreview = @js(isset($parent) && $parent->avatar ? asset('storage/' . $parent->avatar) : '');
                  this.touched = {};
                ...
                <div class="space-y-5">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        <div>
                            ...
                            <x-antarmuka.galat-sebaris name="no_telepon_orangtua" />
                        </div>
                    </div>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                            <label class="block text-sm text-gray-600 font-semibold mb-2">Password {!! isset($parent) ? '<span class="text-xs font-normal text-gray-500 ml-1">(Kosongkan jika tidak diubah)</span>' : '<span class="text-red-500">*</span>' !!}</label>
                            <div class="relative">
                                <input :type="showPassword ? 'text' : 'password'" name="password" x-model="password" placeholder="Minimal 8 karakter" @blur="touched.password = true" :class="touched.password && password === '' && !{{ isset($parent) ? 'true' : 'false' }} ? 'border-red-500 focus:ring-red-500 bg-red-50/30' : 'border-gray-200 focus:border-primary-400 bg-gray-50'" class="block w-full rounded-2xl p-3 pr-10 border shadow-sm focus:bg-white focus:outline-none focus:ring-2 focus:ring-primary-200 transition-colors duration-100 text-sm font-medium text-gray-800" {{ isset($parent) ? '' : 'required' }}>
                        ...
                        <div>
                            <label class="block text-sm text-gray-600 font-semibold mb-2">Konfirmasi Password {!! isset($parent) ? '' : '<span class="text-red-500">*</span>' !!}</label>
                            <div class="relative">
        ...
        <div class="flex flex-col-reverse md:flex-row gap-4 mt-8">
            ...
            <button type="submit" class="w-full md:w-2/3 flex items-center justify-center px-8 py-4 text-base font-extrabold text-white transition-all duration-100 bg-primary-600 rounded-2xl hover:bg-primary-700 shadow-lg shadow-primary-500/30 transform hover:-translate-y-1">
                ...
                {{ isset($parent) ? 'Simpan Perubahan Data Wali Murid' : 'Simpan Data Wali Murid Baru' }}
            </button>
        </div>
    </form>
</div>
