...
<div class="space-y-6">
    ...
    <x-admin.tajuk-halaman
        title="Daftar Wali Murid Ruang Les"
        description="Jaga komunikasi tetap lancar dengan mengelola data kontak pendamping belajar murid."
        actionUrl="{{ route('admin.parents.create') }}"
        actionLabel="Tambah Wali"
    />
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($parents as $parent)
                        <tr class="hover:bg-gray-50 transition-colors" x-show="search === '' || @js(strtolower($parent->name ?? '')).includes(search.toLowerCase()) || @js(strtolower($parent->email ?? '')).includes(search.toLowerCase())" x-transition>
                            <td class="px-4 py-3 align-middle">
                                <div class="flex items-center">
                                    <div class="flex-shrink-0">
                                        <x-admin.avatar :name="$parent->name" :avatar-url="$parent->avatar" />
                                    </div>
                                    <div class="ml-4">
                            ...
                            <td class="px-4 py-3 align-middle">
                                ...
                                <div class="text-xs text-gray-500 flex items-center mt-1">
                                    ...
                                    {{ $parent->parentProfile->no_telepon_orangtua ?? '-' }}
                                </div>
                            </td>
                            <td class="px-4 py-3 align-middle text-sm font-semibold text-gray-900">
                                {{ optional($parent->parentProfile)->status_hubungan ?? '-' }}
                            </td>
                            <td class="px-4 py-3 align-middle">
                                <x-antarmuka.lencana color="primary" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold border w-fit">
                                    {{ $parent->students->count() }} Anak
                                </x-antarmuka.lencana>
                            </td>
                            <td class="px-4 py-3 align-middle text-center whitespace-nowrap">
                                <div class="flex items-center justify-center space-x-2">
                                    <a href="{{ route('admin.parents.show', $parent->id) }}" class="inline-flex items-center p-2 text-sm font-medium text-gray-500 bg-gray-50 rounded-lg hover:bg-gray-100 hover:text-primary-600 transition-colors focus:outline-none focus:ring-2 focus:ring-primary-500" title="Lihat Detail">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                    </a>
                                    <a href="{{ route('admin.parents.edit', $parent->id) }}" class="inline-flex items-center p-2 text-sm font-medium text-gray-500 bg-gray-50 rounded-lg hover:bg-gray-100 hover:text-primary-600 transition-colors focus:outline-none focus:ring-2 focus:ring-primary-500" title="Edit">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                    </a>
                                    <x-admin.formulir-hapus :route="route('admin.parents.destroy', $parent->id)" :item-name="$parent->name" />
                                </div>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-12 align-middle">
                                <x-admin.keadaan-kosong
                                    ...
                                    message="Saat ini belum ada wali murid yang bergabung maupun ditambahkan."
                                />
                            </td>
                        </tr>
                    @endforelse
