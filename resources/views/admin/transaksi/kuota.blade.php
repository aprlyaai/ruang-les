...
@section('breadcrumbs')
    ...
    <a href="{{ route('admin.transactions.index') }}" class="text-gray-500 hover:text-gray-700">Pembayaran</a>
    <svg class="w-3 h-3 mx-2 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
...
<div class="bg-white rounded-2xl shadow-sm border border-primary-100/50 overflow-hidden mb-8"
    ...
    <div class="p-6 bg-white border-b border-gray-100">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <a href="{{ route('admin.transactions.kuota', ['quota_status' => 'all']) }}" class="bg-primary-50/50 rounded-xl p-5 border border-primary-100 flex items-center justify-between hover:shadow-md hover:-translate-y-1 transition-all cursor-pointer group">
                <div>
                    ...
                    <p class="text-2xl font-bold text-primary-900 mt-1">{{ $totalActive }}</p>
                </div>
            ...
            <a href="{{ route('admin.transactions.kuota', ['quota_status' => 'kritis']) }}" class="bg-yellow-50/50 rounded-xl p-5 border border-yellow-100 flex items-center justify-between hover:shadow-md hover:-translate-y-1 transition-all cursor-pointer group">
                <div>
                    ...
                    <p class="text-2xl font-bold text-yellow-700 mt-1">{{ $totalZeroQuota }}</p>
                </div>
            ...
            <a href="{{ route('admin.transactions.kuota', ['quota_status' => 'nunggak']) }}" class="bg-red-50 rounded-xl p-5 border border-red-100 flex items-center justify-between hover:shadow-md hover:-translate-y-1 transition-all cursor-pointer group">
                <div>
                    ...
                    <p class="text-2xl font-bold text-red-700 mt-1">{{ $totalNegativeQuota }}</p>
                </div>
...
<!-- Toolbar (Filter & Search) -->
    <div class="p-5 border-b border-gray-100 bg-white">
        <form action="{{ route('admin.transactions.kuota') }}" method="GET" class="w-full">
            <div class="flex flex-col md:flex-row items-end gap-4">
                ...
                <div class="w-full md:w-1/4">
                    ...
                    <select name="quota_status" class="block w-full pl-3 pr-10 py-2 text-sm font-medium text-gray-700 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-200 focus:border-primary-400 bg-gray-50 hover:bg-white transition-colors shadow-sm appearance-none bg-[url('data:image/svg+xml;charset=US-ASCII,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%2224%22%20height%3D%2224%22%20viewBox%3D%220%200%2024%2024%22%20fill%3D%22none%22%20stroke%3D%22%234b5563%22%20stroke-width%3D%222%22%20stroke-linecap%3D%22round%22%20stroke-linejoin%3D%22round%22%3E%3Cpolyline%20points%3D%226%209%2012%2015%2018%209%22%3E%3C%2Fpolyline%3E%3C%2Fsvg%3E')] bg-[length:1.25rem_1.25rem] bg-[position:right_0.75rem_center] bg-no-repeat cursor-pointer">
                        <option value="all" {{ request('quota_status') == 'all' ? 'selected' : '' }}>Semua Murid</option>
                        <option value="aman" {{ request('quota_status') == 'aman' ? 'selected' : '' }}>Aman (> 0)</option>
                        <option value="kritis" {{ request('quota_status') == 'kritis' ? 'selected' : '' }}>Batas Kritis (= 0)</option>
                        <option value="nunggak" {{ request('quota_status') == 'nunggak' ? 'selected' : '' }}>Menunggak (< 0)</option>
                    </select>
                </div>
                ...
                <div class="flex justify-end items-center gap-3 w-full md:w-auto mt-2 md:mt-0">
                    <a href="{{ route('admin.transactions.kuota') }}" class="inline-flex items-center justify-center px-4 py-2 text-sm font-bold text-gray-700 bg-white border border-gray-300 rounded-xl hover:bg-gray-50 transition-all shadow-sm whitespace-nowrap">
                        Reset Filter
                    </a>
        ...
        <table class="w-full text-left border-collapse text-sm">
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50/80 transition-colors">
                    <td class="px-6 py-4">
                        <p class="text-sm font-bold text-gray-900">{{ $student->nama_murid }}</p>
                        <p class="text-xs text-gray-500 mt-1">Panggilan: {{ $student->panggilan_murid }}</p>
                    </td>
                    <td class="px-6 py-4">
                        <p class="text-sm font-bold text-gray-900">{{ $student->user->name ?? 'N/A' }}</p>
                        <p class="text-xs text-gray-500 mt-1">{{ $student->user->email ?? '-' }}</p>
                    </td>
                    <td class="px-4 py-3 align-middle text-center">
                        <!-- Ukuran lingkaran disesuaikan (w-8 h-8) agar lebih proporsional dengan tinggi baris -->
                        @if($isNegative)
                            <x-antarmuka.lencana color="danger" class="inline-flex items-center justify-center w-8 h-8 rounded-full font-bold text-sm shadow-sm border">
                                {{ $student->kuota_belajar }}
                            </x-antarmuka.lencana>
                        @elseif($isZero)
                            <x-antarmuka.lencana color="warning" class="inline-flex items-center justify-center w-8 h-8 rounded-full font-bold text-sm shadow-sm border">
                                {{ $student->kuota_belajar }}
                            </x-antarmuka.lencana>
                        @else
                            <x-antarmuka.lencana color="primary" class="inline-flex items-center justify-center w-8 h-8 rounded-full font-bold text-sm shadow-sm border">
                                {{ $student->kuota_belajar }}
                            </x-antarmuka.lencana>
                        @endif
                    </td>
                    <td class="px-4 py-3 align-middle text-center">
                        @if($isNegative)
                            ...
                            <button @click="showModal = true; selectedStudentId = {{ $student->id }}; selectedStudentName = '{{ $student->nama_murid }}'" class="inline-flex items-center justify-center px-3 py-1.5 text-xs font-bold text-white transition-all duration-300 bg-red-600 rounded-lg hover:bg-red-700 shadow-sm hover:shadow-md hover:-translate-y-0.5 animate-pulse w-fit">
                                Kirim Teguran
                            </button>
                        @elseif($isZero)
                            <!-- Label diubah menggunakan style badge agar konsisten dengan Kode 1 -->
                            <x-antarmuka.lencana color="warning">Jatuh Tempo</x-antarmuka.lencana>
                        @else
                            <x-antarmuka.lencana color="primary">Aman</x-antarmuka.lencana>
                        @endif
                    </td>
                </tr>
                @empty
        ...
        <div x-cloak x-show="showModal" class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
            ...
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div x-show="showModal"
                    ...
                    <form action="{{ route('admin.transactions.remind') }}" method="POST">
                        @csrf
                        ...
                        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            <h3 class="text-xl leading-6 font-bold text-gray-900 flex items-center">
                                <x-antarmuka.lencana color="danger" class="w-10 h-10 rounded-full flex items-center justify-center mr-3">
                                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path></svg>
                                </x-antarmuka.lencana>
