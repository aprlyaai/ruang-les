...
<div class="w-full space-y-6">
    ...
    <form action="{{ route('mentor.profile.update') }}" method="POST" enctype="multipart/form-data"
          x-data="{
              ...
              phoneNumber: @js((string) old('no_telepon_mentor', $user->mentorProfile->no_telepon_mentor ?? '')),
              showPassword: false,
              showPasswordConfirm: false,
              password: '',
              birthPlace: @js((string) old('tempat_lahir_mentor', $user->mentorProfile->tempat_lahir_mentor ?? '')),
              birthDate: @js((string) old('tanggal_lahir_mentor', $user->mentorProfile->tanggal_lahir_mentor ?? '')),
              jenis_kelamin_mentor: @js((string) old('jenis_kelamin_mentor', $user->mentorProfile->jenis_kelamin_mentor ?? '')),
              alamat_mentor: @js((string) old('alamat_mentor', $user->mentorProfile->alamat_mentor ?? '')),
              education: @js((string) old('pendidikan_mentor', $user->mentorProfile->pendidikan_mentor ?? '')),
              ...
              resetForm() {
                  ...
                  this.phoneNumber = @js((string) old('no_telepon_mentor', $user->mentorProfile->no_telepon_mentor ?? ''));
                  this.password = '';
                  this.showPassword = false;
                  this.showPasswordConfirm = false;
                  this.birthPlace = @js((string) old('tempat_lahir_mentor', $user->mentorProfile->tempat_lahir_mentor ?? ''));
                  this.birthDate = @js((string) old('tanggal_lahir_mentor', $user->mentorProfile->tanggal_lahir_mentor ?? ''));
                  this.jenis_kelamin_mentor = @js((string) old('jenis_kelamin_mentor', $user->mentorProfile->jenis_kelamin_mentor ?? ''));
                  this.alamat_mentor = @js((string) old('alamat_mentor', $user->mentorProfile->alamat_mentor ?? ''));
                  this.education = @js((string) old('pendidikan_mentor', $user->mentorProfile->pendidikan_mentor ?? ''));
                  ...
                  setTimeout(() => {
                      ...
                      if (ts) ts.value = @js((string) old('spesialisasi_mentor', $user->mentorProfile->spesialisasi_mentor ?? ''));

                      let bn = document.querySelector('input[name=nama_bank]');
                      if (bn) bn.value = @js((string) old('nama_bank', $user->mentorProfile->nama_bank ?? ''));

                      let ban = document.querySelector('input[name=nomor_akun_bank]');
                      if (ban) ban.value = @js((string) old('nomor_akun_bank', $user->mentorProfile->nomor_akun_bank ?? ''));

                      let bname = document.querySelector('input[name=nama_akun_bank]');
                      if (bname) bname.value = @js((string) old('nama_akun_bank', $user->mentorProfile->nama_akun_bank ?? ''));
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <div>
                                ...
                                <select name="jenis_kelamin_mentor" x-model="jenis_kelamin_mentor" @blur="touched.jenis_kelamin_mentor = true" :class="touched.jenis_kelamin_mentor && jenis_kelamin_mentor.trim() === '' ? 'border-red-500 focus:ring-red-500 bg-red-50/30' : 'border-gray-200 focus:border-primary-400 bg-gray-50'" class="block w-full appearance-none rounded-2xl p-3 pr-10 border shadow-sm focus:bg-white focus:outline-none focus:ring-2 focus:ring-primary-200 transition-colors duration-100 text-sm font-medium text-gray-800 bg-[url('data:image/svg+xml;charset=US-ASCII,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%2224%22%20height%3D%2224%22%20viewBox%3D%220%200%2024%2024%22%20fill%3D%22none%22%20stroke%3D%22%234b5563%22%20stroke-width%3D%222%22%20stroke-linecap%3D%22round%22%20stroke-linejoin%3D%22round%22%3E%3Cpolyline%20points%3D%226%209%2012%2015%2018%209%22%3E%3C%2Fpolyline%3E%3C%2Fsvg%3E')] bg-[length:1.25rem_1.25rem] bg-[position:right_1rem_center] bg-no-repeat cursor-pointer" required>
                                    ...
                                    <option value="Laki-laki" {{ old('jenis_kelamin_mentor', $user->mentorProfile->jenis_kelamin_mentor ?? '') == 'Laki-laki' ? 'selected' : '' }}>Laki-laki</option>
                                    <option value="Perempuan" {{ old('jenis_kelamin_mentor', $user->mentorProfile->jenis_kelamin_mentor ?? '') == 'Perempuan' ? 'selected' : '' }}>Perempuan</option>
                                </select>
                            ...
                            <div>
                                ...
                                <input type="text" name="no_telepon_mentor" x-model="phoneNumber" placeholder="Contoh: 08123456789" @blur="touched.phoneNumber = true" :class="touched.phoneNumber && phoneNumber.trim() === '' ? 'border-red-500 focus:ring-red-500 bg-red-50/30' : 'border-gray-200 focus:border-primary-400 bg-gray-50'" class="block w-full rounded-2xl p-3 border shadow-sm focus:bg-white focus:outline-none focus:ring-2 focus:ring-primary-200 transition-colors duration-100 text-sm font-medium text-gray-800" required>
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-primary-100/50 p-5 md:p-6">
                        ...
                        <div class="space-y-5">
                            ...
                            <div>
                                ...
                                <input type="text" name="spesialisasi_mentor" value="{{ old('spesialisasi_mentor', $user->mentorProfile->spesialisasi_mentor ?? '') }}" placeholder="Contoh: Mata pelajaran SD" class="block w-full rounded-2xl p-3 border border-gray-200 bg-gray-50 shadow-sm focus:bg-white focus:outline-none focus:ring-2 focus:ring-primary-200 focus:border-primary-400 transition-colors duration-100 text-sm font-medium text-gray-800">
                            </div>
                        </div>
                    </div>
                </div>
