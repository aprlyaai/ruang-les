...
<div class="space-y-6" x-data="{ previewModalOpen: false, previewTitle: '', previewUrl: '', isImage: false }">
    ...
    <x-admin.tajuk-halaman
        title="Materi Belajar & Silabus"
        description="Akses modul, silabus, dan materi pembelajaran dari seluruh jenjang yang disediakan oleh Admin."
    />
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-primary-100/50 overflow-hidden mb-6 mt-6">
        ...
        <div class="p-5">
            <form action="{{ route('mentor.materi.index') }}" method="GET">
                ...
                <div class="mt-4 flex flex-col-reverse sm:flex-row sm:justify-end gap-3">
                    <a href="{{ route('mentor.materi.index') }}" class="inline-flex items-center justify-center px-5 py-2.5 text-sm font-bold text-gray-700 transition-all duration-100 bg-white border border-gray-300 rounded-xl hover:bg-gray-100 focus:outline-none">
                        Reset Filter
                    </a>
    ...
    <div class="mt-6">
        @if($materials->isEmpty())
            <div class="bg-white rounded-2xl shadow-sm border border-primary-100/50 overflow-hidden">
                <x-admin.keadaan-kosong
                    ...
                    message="Belum ada materi belajar yang diunggah atau tidak ada data yang cocok dengan kriteria filter Anda."
                >
        ...
        @else
                    ...
                    <div class="bg-white rounded-2xl p-5 border border-gray-100 shadow-[0_2px_10px_-3px_rgba(6,81,237,0.05)] hover:shadow-[0_8px_20px_-6px_rgba(6,81,237,0.1)] hover:-translate-y-1 transition-all duration-300 flex flex-col h-full group relative overflow-hidden">
                        ...
                        <div class="mt-auto pt-4 border-t border-gray-50">
                            ...
                            <div class="flex grid grid-cols-2 gap-2 mt-4 transition-all duration-300">
                                ...
                                <a href="{{ route('mentor.materi.show', $material->id) }}" target="_blank"
            ...
            @if($materials->hasPages())
                <div class="mt-8 mb-4">
                    {{ $materials->appends(['search' => $search, 'kelas' => $kelas, 'mapel' => $mapel])->links() }}
                </div>
            @endif
        @endif
    </div>
